...
export const ServerProvider = ({ children }: ServerProviderProps) => {
  ...
  const [modelMetrics, setModelMetrics] = useState<any | null>(null);
    const fetchModelStatus = useCallback(async () => {
      try {
        ...
        if (!response.ok) throw new Error('Error fetching model status');
        ...
        try {
          parsedMetrics = activeModel.metrics ? JSON.parse(activeModel.metrics) : {};
        } catch {
          parsedMetrics = {};
        }
        ...
        setModelStatus(data.status === 'ready' ? 'trained' : 'idle');
      } catch (err) {
  ...
  const trainModel = useCallback(async () => {
    try {
      ...
      toast.success('Modelo de Arbol de Decision entrenado correctamente');
    } catch (err) {
  ...
  const sendTraffic = useCallback(async (trafficData: Partial<TrafficMetrics>) => {
    try {
      if (
        typeof (trafficData as any).signal_strength === 'number' &&
        (((trafficData as any).signal_strength > 0) || ((trafficData as any).signal_strength < -150))
      ) {
        throw new Error('La señal debe estar entre -150 y 0 dBm');
      }

      if (
        typeof (trafficData as any).handover_success === 'number' &&
        ![0, 1].includes((trafficData as any).handover_success)
      ) {
        throw new Error('handover_success debe ser 0 o 1');
      }
      ...
      const errorMessage = err instanceof Error ? err.message : 'Error al enviar tráfico';
  ...
  }, [fetchServers, fetchLogs, fetchModelStatus]);
