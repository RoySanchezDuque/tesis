...
const TrafficSimulator = () => {
  ...
  const handleSendTraffic = async () => {
    ...
    const trafficData = {
      ...
      throughput: parseFloat((Math.random() * 1000).toFixed(2)),
      packet_loss: parseFloat((Math.random() * 5).toFixed(2)),
      signal_strength: parseFloat((-120 + Math.random() * 90).toFixed(2)), // -120 a -30 dBm
      resource_allocation: parseFloat((Math.random() * 100).toFixed(2)),
      handover_success: Math.random() > 0.2 ? 1 : 0
    };
