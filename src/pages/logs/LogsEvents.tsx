...
const LogsEvents = () => {
  const { logs, fetchLogs } = useServer();
  const [filteredLogs, setFilteredLogs] = useState<EventoLog[]>([]);
  ...
  const normalizedLogs: EventoLog[] = logs.map((log: any) => {
    const isError = (log.packet_loss ?? 0) >= 2.5 || (log.network_latency ?? 0) >= 180;
    const isWarning = !isError && ((log.packet_loss ?? 0) >= 1.5 || (log.network_latency ?? 0) >= 120);
    const severity: 'info' | 'warning' | 'error' = isError ? 'error' : isWarning ? 'warning' : 'info';

    return {
      id: log.id,
      timestamp: log.timestamp,
      tipo: 'balanceo',
      mensaje: log.assigned_server_name
        ? `Asignado a ${log.assigned_server_name} | Tráfico ${(log.traffic_volume ?? 0).toFixed(2)} Mbps | Latencia ${(log.network_latency ?? 0).toFixed(2)} ms | Pérdida ${(log.packet_loss ?? 0).toFixed(2)}%`
        : 'Evento de tráfico registrado',
      nodoId: log.assigned_server_id ?? null,
      severidad: severity,
    };
  });
  ...
  }, [normalizedLogs, searchTerm, selectedSeverity, selectedType, selectedNode]);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card p-4 flex items-center">
          ...
          <div>
            ...
            <p className="text-xl font-semibold text-gray-800 dark:text-white">{normalizedLogs.length}</p>
          </div>
        </div>
        ...
        <div className="card p-4 flex items-center">
          ...
          <div>
            ...
            <p className="text-xl font-semibold text-gray-800 dark:text-white">
              {normalizedLogs.filter(e => e.severidad === 'error').length}
            </p>
          </div>
        </div>
        ...
        <div className="card p-4 flex items-center">
          ...
          <div>
            ...
            <p className="text-xl font-semibold text-gray-800 dark:text-white">
              {normalizedLogs.filter(e => e.severidad === 'warning').length}
            </p>
          </div>
        </div>
        ...
        <div className="card p-4 flex items-center">
          ...
          <div>
            ...
            <p className="text-xl font-semibold text-gray-800 dark:text-white">
              {normalizedLogs.filter(e => e.tipo === 'balanceo').length}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="flex justify-between items-center mb-4">
          ...
          <span className="text-sm text-gray-500 dark:text-gray-400">
            Mostrando {filteredLogs.length} de {normalizedLogs.length} eventos
          </span>
        </div>
