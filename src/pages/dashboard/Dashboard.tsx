...
const Dashboard = () => {
  ...
  const eventosRecientesNormalizados = logs.slice(0, 5).map((log: any, idx: number) => ({
    id: log.id ?? idx,
    // LogsPanel applies its own formatting, so this must stay as a parseable date string.
    timestamp: log.timestamp ?? new Date().toISOString(),
    tipo: 'log',
    mensaje:
      log.assigned_server_name
        ? `Tráfico asignado a ${log.assigned_server_name} (${(log.traffic_volume ?? 0).toFixed?.(2) ?? log.traffic_volume} Mbps)`
        : 'Evento del servidor',
